...
function App() {
  ...
  function begin(mode: 'practice' | 'simulation') {
    const ordered = [...questions].sort((a, b) => (Number(!reviewIds.has(a.id)) - Number(!reviewIds.has(b.id))) || a.id.localeCompare(b.id))
    const selectedQuestions = ordered.slice(0, mode === 'simulation' ? 10 : 5)
  ...
  async function submitAnswer() {
    if (!current || !selected || !confidence || submitted || !session) return
    const attempt: Attempt = { questionId: current.id, attemptedAt: new Date().toISOString(), selectedOptionId: selected, correct: selected === current.correctOptionId, confidence, responseTimeSeconds: 0, mode: session.mode, examId: null }
    await saveAttempt(attempt); setAttempts((previous) => [...previous, attempt]); setSession({ ...session, correct: session.correct + (attempt.correct ? 1 : 0) }); setSubmitted(true)
  }
