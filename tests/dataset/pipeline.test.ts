...
describe('pipeline con el dataset real', () => {
  it('conserva inventario, hashes y fuentes pendientes', async () => {
    ...
    expect(result.data?.sources).toHaveLength(45)
    expect(result.data?.inventory).toHaveLength(45)
    expect(result.data?.sourceUnits).toHaveLength(28)
  ...
  it('reporta ceros explícitos y cobertura por tema y convocatoria', async () => {
    ...
    expect(report).toContain('| pending_review | 23 |')
